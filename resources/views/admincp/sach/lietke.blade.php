    ...
    <tbody>
        ...
        @foreach ($sach as $item)
            <tr>
                ...
                <td><img src="{{ asset('Assets/image/HinhAnhSach/'.$item->hinhanh) }}" style="border-radius: 50%;" alt="" height="50" width="50" srcset=""></td>
                <td>{{ $item->mota }}</td>
                <td>
                ...
                <td>
                    <a class="btn btn-primary" href="{{ route('sach.edit', [$item->ID]) }}">Edit</a>
                    <form action="{{ route('sach.destroy', ['sach' => $item->ID]) }}" method="POST">
                        @method('DELETE')
                        @csrf
